...
proc do_worth {action fiat nick chan param} {
  ...
  if {[dashircbot_refresh_tablevar]} {
    ...
    set mcapsupply [dashircbot_tablevar_fetch "marketcapsupply"]
    if { [lindex $mcapsupply 0] == false } {
      dashircbot_unavailable $header $lang
      return
    }
    puts 2
    set mcapvolbtc [dashircbot_tablevar_fetch "volumebtc"]
    if { [lindex $mcapvolbtc 0] == false } {
      dashircbot_unavailable $header $lang
      return
    }
    set mcapvolusd [dashircbot_tablevar_fetch "volumeusd"]
    if { [lindex $mcapvolusd 0] == false } {
      dashircbot_unavailable $header $lang
      return
    }
    set mcapvoleur [dashircbot_tablevar_fetch "volumeeur"]
    if { [lindex $mcapvoleur 0] == false } {
      dashircbot_unavailable $header $lang
      return
    }
    set mcapchange [dashircbot_tablevar_fetch "marketcapchange"]
...
lappend dashircbot_command_fr { {!marketcapeur & !marketcapusd} {Capitalisation du March�} }
